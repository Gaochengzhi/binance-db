...
def convert_one(csv_path: pathlib.Path):
    ...
    try:
        ...
        if not m:
            ...
            return {"status": "skipped", "path": str(csv_path), "reason": "filename pattern mismatch"}
        ...
        # 根据文件路径和第二个字段确定数据类型
        if any(kline_type in str(csv_path) for kline_type in ["klines", "indexPriceKlines", "markPriceKlines", "premiumIndexKlines"]):
            # K线数据: kind_or_interval 是时间间隔
            interval = kind_or_interval
            # 从路径中推断数据类型
            for kline_type in ["klines", "indexPriceKlines", "markPriceKlines", "premiumIndexKlines"]:
        ...
        if parquet_path.exists():
            try:
                ...
                return {"status": "skipped", "path": str(csv_path), "reason": "already exists and valid"}
            except Exception:
                # 文件损坏，删除重新创建
        ...
        # 读取 CSV → Arrow Table
        table = pv.read_csv(csv_path, read_options=pv.ReadOptions(autogenerate_column_names=False))
        ...
        # 写 Parquet，保持按时间排序
        sorter = table.column_names.index('open_time') if 'open_time' in table.column_names else None
        if sorter is not None:
            indices = pa.compute.sort_indices(table, sort_keys=[('open_time', 'ascending')])
...
def init_duck_views():
    ...
    for v, path in views.items():
        con.execute(f"CREATE OR REPLACE VIEW {v} AS SELECT * FROM parquet_scan('{path}');")
    con.close()
...
class BinanceDuck:
    ...
    def scan_15m_optimized(self, start_ts, end_ts, symbols=None, table="klines"):
        ...
        while current_date <= end_date:
            file_patterns.append(f"'{DATA_PATH}/{table}/interval=15m/date={current_date}/*.parquet'")
    ...
    def scan_single_symbol(self, symbol, start_ts, end_ts, table="klines"):
        ...
        file_patterns = []
        while current_date <= end_date:
            file_patterns.append(f"'{DATA_PATH}/{table}/interval=15m/date={current_date}/symbol={symbol}.parquet'")
            current_date += timedelta(days=1)
...
def get_data_status():
    ...
    if parquet_files:
        ...
        print(f"📊 转换进度: {len(parquet_files)}/{len(csv_files)} ({len(parquet_files)/len(csv_files)*100:.1f}%)")
    ...
    try:
        ...
        test_scenarios = [
            {
                ...
                'func': lambda: db.scan_15m_optimized(TEST_SHORT_START_MS, TEST_SHORT_END_MS),
                'type': '数据密集型'
            },
            {
                'name': '单符号长时序',  
                'desc': f'BTCUSDT的{(TEST_LONG_END - TEST_LONG_START).days}天时间序列',
                'func': lambda: db.scan_single_symbol('BTCUSDT', TEST_LONG_START_MS, TEST_LONG_END_MS),
                'type': '时间跨度型'
            },
            {
                'name': '热门符号扫描',
            ...
            },
            {
                'name': '交易量统计',
                'desc': '聚合分析交易量排名',
                'func': lambda: db.scan_top_volume(TEST_SHORT_START_MS, TEST_SHORT_END_MS, limit=20),
                'type': '聚合分析型'
            }
        ]
        ...
        for scenario in test_scenarios:
            ...
            try:
                ...
                print(f"{scenario['name']:<15} {scenario['type']:<12} {query_time:.2f}s {count:<10,} {rps:>8,.0f}/s")
                
            except Exception as e:
                query_time = time.time() - query_start
                print(f"{scenario['name']:<15} {scenario['type']:<12} {query_time:.2f}s {'失败':<10} {str(e)[:20]}")
